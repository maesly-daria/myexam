...
from .models import Post, Client, House, Booking, Service, CustomUser, Review
...
class CustomUserCreationForm(UserCreationForm):
    ...
    class Meta:
        model = CustomUser
        fields = ('username', 'email', 'phone', 'last_name', 'patronymic', 'password1', 'password2')
...
class ClientForm(forms.ModelForm):
    class Meta:
        model = Client
        fields = ['last_name', 'first_name', 'patronymic', 'email', 'phone_number', 'document']
        widgets = {
            'last_name': forms.TextInput(attrs={'class': 'form-control'}),
...
class BookingForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.user and self.user.is_authenticated:
            ...
            self.fields['client_name'].initial = ' '.join(part for part in name_parts if part).strip()
    ...
    def _get_full_name(self):
        ...
        if self.user:
            parts = [self.user.last_name, self.user.username, self.user.patronymic or '']
